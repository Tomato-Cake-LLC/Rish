...
namespace RishUI
{
    ...
    public partial class Node : FastPriorityQueueNode
    {
        ...
        internal SapTargets OnMounted => OnMountedStem.Targets;
        private SapStem OnBeforeUnmountStem { get; } = new();
        internal SapTargets OnBeforeUnmount => OnBeforeUnmountStem.Targets;
        private SapStem<Node> OnInactiveStem { get; } = new(); // TODO: Maybe uint?
        internal SapTargets<Node> OnInactive => OnInactiveStem.Targets;
        
        private SapStem<VisualElement> OnVisualElementAttachedStem { get; } = new();
        public SapTargets<VisualElement> OnVisualElementAttached => OnVisualElementAttachedStem.Targets;
        private SapStem<VisualElement> OnVisualElementDetachedStem { get; } = new();
        public SapTargets<VisualElement> OnVisualElementDetached => OnVisualElementDetachedStem.Targets;
        ...
        internal VisualManipulator VisualManipulator { get; private set; }
        ...
        internal int GetVisualSize()
        {
            var count = Children?.Count(n => n.IsVisualElement) ?? 0;
            if(Children != null)
            {
                foreach (var child in Children)
                {
                    count += child.GetVisualSize();
                }
            }
        ...
        private Node(int id)
        {
            ...
            Machine.OnChange.Add(Sappy.OnStateChange);
        }
...
// #endif
        ...
        [SapTarget]
        internal void Dirty(bool immediate)
        {
#if UNITY_EDITOR
            if (Tree == null)
            {
        ...
        private class ReadyToMountState : State
        {
            ...
            public override T MountAs<T>(Node parent, ulong key, int index)
            {
                ...
                var parentVisualManipulator = parent?.VisualManipulator;
                VisualManipulator visualManipulator;
                if (element is IRishElement rishElement and IVisualManipulator)
                {
                    visualManipulator = rishElement.VisualManipulator;
                    visualManipulator.SetParent(parentVisualManipulator);
                }
                else
                {
                    visualManipulator = parentVisualManipulator;
                }
                Node.VisualManipulator = visualManipulator;

                GoTo<MountedState>();
                
                // TODO: Report mount for layout calculations
        ...
        private class MountedState : ActiveState
        {
            ...
            public override void Enter()
            {
                switch (Node.Element)
                {
                    case IRishElement rishElement:
                        rishElement.OnDirty.Add(Node.Sappy.Dirty);
            ...
            public override void Exit()
            {
                ...
                {
                    case IRishElement rishElement:
                        rishElement.OnDirty.Remove(Node.Sappy.Dirty);
                        break;
                    case IInternalVisualElement visualElement:
        ...
        private partial class UnmountRequestedState : ActiveState
        {
            ...
            public override void Enter()
            {
                ...
                {
                    rishElement.OnDirty.Add(Node.Sappy.Dirty);
                    rishElement.OnReadyToUnmount.Add(Sappy.ElementReadyToUnmount);
            ...
            public override void Exit()
            {
                ...
                {
                    rishElement.OnDirty.Remove(Node.Sappy.Dirty);
                    rishElement.OnReadyToUnmount.Remove(Sappy.ElementReadyToUnmount);
                }
                ElementReady = false;
                ...
                if (Unmounting != null)
                {
                    for (int i = 0, n = Unmounting.Count; i < n; i++)
                    {
                        ...
                        child.Machine.OnChange.Remove(Sappy.OnStateChange);
                    }
                ...
                if (unmountingCount > 0)
                {
                    ...
                    for (var i = 0; i < unmountingCount; i++)
                    {
                        ...
                        if (UnmountingSet.Add(childId))
                        {
                            ...
                            child.Machine.OnChange.Add(Sappy.OnStateChange);
                        }
#if UNITY_EDITOR
                        else
                        {
