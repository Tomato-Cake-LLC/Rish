...
namespace RishUI
{
    ...
    public class RishRoot : MonoBehaviour
    {
        ...
        public SapTargets OnStart => OnStartStem.Targets;
        
#if UNITY_EDITOR
        private SapStem<RishRoot> OnStepStem { get; } = new();
        public SapTargets<RishRoot> OnStep => OnStepStem.Targets;
#endif
        ...
        [SerializeField]
        private bool _shouldRecordStepTime;
        public bool ShouldRecordStepTime {
            get => _shouldRecordStepTime || MaxTargetTimePerStep > 0;
            set => _shouldRecordStepTime = value;
        }
...
#if UNITY_EDITOR
        public int GetTreeSize() => Tree?.GetSize() ?? 0;
        public int GetVisualTreeSize() => Tree?.GetVisualSize() ?? 0;
#endif
        ...
        public void Step()
        {
            ...
            {
                maxUpdateTime = ShouldRecordStepTime ? 0 : null;
            }
            ...
            if (!ShouldRecordStepTime || !updateTime.HasValue) return;
        ...
        private void RecordStepTime(float stepTime)
        {
            if (RecordedStepTimes.Count >= RecordedStepsCount)
            {
                TotalRecordedStepTime -= RecordedStepTimes.Dequeue();
            }
            TotalRecordedStepTime += stepTime;
            RecordedStepTimes.Enqueue(stepTime);
        }
