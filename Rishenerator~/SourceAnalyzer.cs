...
namespace Rishenerator
{
    ...
    public class SourceAnalyzer : DiagnosticAnalyzer
    {
        ...
        private static readonly DiagnosticDescriptor ExpandTypeRule = new(
            "ExpandTypeRule",
            "Invalid type for Expand attribute",
            "Field {0} can't have the Expand attribute",
            "Usage",
            DiagnosticSeverity.Error,
            true,
            "At the moment, Expand attribute is only valid on VisualAttributes fields.",
            null // TODO: Help link to documentation
        );
            ...
            if (structSymbol.IsFlaggedAsRishValueType())
            {
                ...
                else
                {
                    foreach (var memberSymbol in structSymbol.GetMembers())
                    {
                        ...
                        if (memberSymbol.HasAttribute("RishUI.ExpandAttribute"))
                        {
                            if (fieldSymbol.Type.GetFullName(false) != "RishUI.VisualAttributes")
                            {
                                var fieldDeclaration = fieldSymbol.DeclaringSyntaxReferences.FirstOrDefault()?.GetSyntax();
                                var diagnostic = Diagnostic.Create(ExpandTypeRule, fieldDeclaration.GetLocation(), fieldSymbol);
                                context.ReportDiagnostic(diagnostic);
                            }
                        }
                    }
        ...
        //             if (interfaceName == "RishUI.MemoryManagement.IPointer")
        //             {
