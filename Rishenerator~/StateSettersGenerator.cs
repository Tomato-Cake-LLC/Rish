...
{{");
                ...
                if (stateItems.NeedsDefaultProvider)
                {
                    sourceCode.AppendLine(@$"    [RishUI.Default]
    private static {stateTypeSymbol.GetFullName(true)} Auto{stateTypeSymbol.Name}DefaultValue => new()
    {{");
    ...
    {{
        ...
        private global::Sappy.SapTarget<System.Action<{stateTypeSymbol.GetFullName(true)}>> _setState;
        public global::Sappy.SapTarget<System.Action<{stateTypeSymbol.GetFullName(true)}>> SetState => _setState ??= new global::Sappy.SapTarget<System.Action<{stateTypeSymbol.GetFullName(true)}>>(Element.SetState);
    ...
    protected partial class SappyStateHolder
    {{
        private global::Sappy.SapTarget<System.Action<{item.TypeFullName}>> _{setterName};
        public global::Sappy.SapTarget<System.Action<{item.TypeFullName}>> {setterName} => _{setterName} ??= new global::Sappy.SapTarget<System.Action<{item.TypeFullName}>>(Element.{setterName});
    }}
    ...
    //                 }
        ...
        ClaimContext({initialIndex}, {ctxName});");
                ...
                public StateItem(IFieldSymbol fieldSymbol)
                {
                    ...
                    if (fieldSymbol.HasAttribute("RishUI.DefaultValueAttribute"))
                    {
                        ...
                        if (!arguments.IsEmpty)
                        {
                            ...
                            if (!argument.IsNull)
                            {
                                var defaultValue = argument.ToCSharpString();
                    ...
                    } else if (fieldSymbol.HasAttribute("RishUI.DefaultCodeAttribute"))
                    {
                        ...
                        if (!argument.IsNull)
                        {
                            ...
                            {
                                var defaultValue = code.Substring(1, code.Length - 2);
                                if (defaultValue != "default")
                                {
                                    DefaultValue = defaultValue;
                                }
            ...
            private class ItemizedState
            {
                ...
                public bool NeedsDefaultProvider { get; }
                ...
                public ItemizedState(ITypeSymbol stateTypeSymbol)
                {
                    foreach (var stateMemberSymbol in stateTypeSymbol.GetMembers())
                    {
                        ...
                        NeedsDefaultProvider |= !string.IsNullOrWhiteSpace(item.DefaultValue);
                    }
